...
Launcher.prototype.start = function (file,opts) {
  ...
  .then(entry => {
    ...
    if(entry['DBusActivatable'] == 'true') {
      var id = entry['ID'].replace(".desktop", ""); //Transform desktop id to service name (service.name)
      var sessionBus = dbus.sessionBus();
      var service = sessionBus.getService(id);
      ...
      service.getInterface("/" + id.replace(/\./g, '/'), 'org.freedesktop.Application', (err, call) => {
      ...
      sessionBus.getService("org.freedesktop.DBus").getInterface("/org/freedesktop/DBus", "org.freedesktop.DBus", (err, call) => {
        call.on("NameOwnerChanged", (name, old_owner, new_owner) => {
          if(name == id && !new_owner)
