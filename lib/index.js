 ...
 */
  ...
  getInterface(id) {
    ...
    return new Promise((resolve, reject) => {
      service.getInterface(uri, "org.freedesktop.Application", (err, call) => {
        if(err) {
          reject(new Error(`Failed to request interface 'org.freedesktop.Application' at '${uri}' : ${err}`));
        } else {
          resolve(call);
        }
      })
  ...
  async openDbus(file, id){
    if(!id || !id.endsWith(".desktop")) {
      this.emit("error", `Invalid id when trying to call open dbus: ${id}`);
      this.emit("end");
      return;
    }
    ...
    try {
      let call = await this.getInterface(id);
      let files = file;
      //Open method need an array in param
      if(!Array.isArray(file)) {
        files = [file];
      }
      await call.Open(files, {"desktop-startup-id" : ["s", "truc"]}, (err) => {
